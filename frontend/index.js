function moduleProject3() {
  ...
  function buildLearnerCard(learner, languages) {
    ...
    [nameP, idElement, dobP, favLangP].forEach(p => {card.appendChild(p)})
  ...
  for (let platform in footerData.socialMedia){
    let socialMediaLink = document.createElement('a')//create anchor for each platform
    socialMediaLink.href = footerData.socialMedia[platform]//give each link href attribute
    socialMediaLink.textContent = platform.charAt(0).toUpperCase()+platform.slice(1)//text content for links
    socialMediaDiv.appendChild(socialMediaLink)//finish loop, append anchor tag to div
  }

    let currentYear = new Date().getFullYear()//get full year
    let copyright = document.createElement('div')//create div for copyright info
